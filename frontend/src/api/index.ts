...
export async function generateTopicIdeas(
  topic: string,
  count: number = 5
): Promise<{ success: boolean; ideas?: TopicIdea[]; raw?: string; error?: string }> {
  try {
    ...
    if (axios.isAxiosError(error)) {
      return {
        success: false,
        error: error.response?.data?.error || error.message || '生成选题方案失败'
      }
    }
...
export function getImageUrl(taskId: string, filename: string, thumbnail: boolean = true): string {
  ...
  return withAuthUrl(`${API_BASE_URL}/images/${taskId}/${filename}${thumbParam}`)
}
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      return {
        success: false,
        index: page.index,
        error: error.response?.data?.error || error.message || '重新生成图片失败'
      }
    }
    return {
      success: false,
...
export async function repairHistory(dryRun: boolean = true): Promise<{
  success: boolean
  dry_run?: boolean
  ...
  const response = await axios.post(`${API_BASE_URL}/history/repair`, {
    dry_run: dryRun
  })
  return response.data
}
...
export async function getTaskList(limit: number = 30): Promise<{
  success: boolean
  tasks: ImageTaskItem[]
  error?: string
}> {
...
export async function checkImageHealth(): Promise<{
  success: boolean
  provider?: string
  model?: string
  message?: string
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/config/image-health`, {
      timeout: 35000
    })
    return response.data
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
      return {
        success: false,
        error: error.response?.data?.error || error.message || '图片服务检查失败'
      }
    }
    return { success: false, error: '图片服务检查失败' }
  }
}

export async function runImageProbe(): Promise<{
  success: boolean
  task_id?: string
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/config/image-probe`, {}, {
      timeout: 180000
    })
    return response.data
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
      return {
        success: false,
        error: error.response?.data?.error || error.message || '真实图片探针失败',
        elapsed_seconds: error.response?.data?.elapsed_seconds
      }
    }
    return { success: false, error: '真实图片探针失败' }
  }
}
...
export interface PublishJob {
  ...
  creation_id?: string | null
  platform: string
  status: 'queued' | 'running' | 'ready_for_review' | 'failed' | 'cancelled'
  logs: string
  error?: string | null
...
export async function getPublishAccounts(platform: string = 'xhs'): Promise<{
  success: boolean
  accounts: PublishAccount[]
  error?: string
}> {
  const response = await axios.get(`${API_BASE_URL}/publish/accounts`, {
    params: { platform }
  })
  return response.data
}

export async function createPublishAccount(
  name: string,
  platform: string = 'xhs'
): Promise<{ success: boolean; account_id?: string; error?: string }> {
  const response = await axios.post(`${API_BASE_URL}/publish/accounts`, {
    name,
    platform
  })
  return response.data
...
export async function deletePublishAccount(accountId: string): Promise<{
  ...
  const response = await axios.delete(`${API_BASE_URL}/publish/accounts/${accountId}`)
  return response.data
}
...
export async function createPublishDraft(data: {
  creation_id: string
  account_id: string
  title: string
  body: string
  ...
  const response = await axios.post(`${API_BASE_URL}/publish/drafts`, data)
  return response.data
}

export async function runPublishDraft(draftId: string): Promise<{
  success: boolean
  job_id?: string
  error?: string
}> {
  const response = await axios.post(`${API_BASE_URL}/publish/drafts/${draftId}/run`, {})
  return response.data
}

export async function getPublishJob(jobId: string): Promise<{
  success: boolean
  job?: PublishJob
  error?: string
}> {
  const response = await axios.get(`${API_BASE_URL}/publish/jobs/${jobId}`)
  return response.data
}

export async function getPublishJobs(limit: number = 50): Promise<{
  success: boolean
  jobs: PublishJob[]
  error?: string
}> {
  const response = await axios.get(`${API_BASE_URL}/publish/jobs`, {
    params: { limit }
  })
  return response.data
}

export async function suggestPublishTags(data: {
  title: string
  body: string
}): Promise<{ success: boolean; tags?: string[]; error?: string }> {
  const response = await axios.post(`${API_BASE_URL}/publish/tags/suggest`, data)
  return response.data
}
...
export async function updateUser(
  userId: string,
  data: Partial<Pick<UserItem, 'name' | 'email' | 'role' | 'status' | 'quota_limit' | 'quota_used'>>
): Promise<{ success: boolean; user?: UserItem; error?: string }> {
...
export async function registerUser(data: {
  ...
  email: string
  password: string
}): Promise<{ success: boolean; token?: string; user?: UserItem; error?: string }> {
...
export async function getCurrentUser(): Promise<{ success: boolean; user?: UserItem; error?: string }> {
