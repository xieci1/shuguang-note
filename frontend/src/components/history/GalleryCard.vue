  ...
  <div class="gallery-card">
    ...
    <div class="card-cover" @click="$emit('preview', record.id)">
      ...
      <div class="card-overlay">
        ...
        <button class="overlay-btn publish" @click.stop="$emit('publish', record.id)">
          发布
        </button>
      </div>

      <!-- 状态标识 -->
      <div class="status-badge" :class="badgeStatus">
        {{ statusText }}
      </div>
    </div>
    ...
    <div class="card-footer">
      ...
      <div class="card-meta">
        ...
        <template v-if="showOwner && record.user">
          <span class="dot">·</span>
...
const statusText = computed(() => {
  ...
  return map[badgeStatus.value] || badgeStatus.value
})

const badgeStatus = computed(() => {
  return props.record.is_published ? 'published' : props.record.status
})

const coverImageUrl = computed(() => {
  return props.record.thumbnail && props.record.task_id
    ? getImageUrl(props.record.task_id, props.record.thumbnail)
    : ''
})
