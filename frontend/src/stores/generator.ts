...
import { withAuthUrl, type Page } from '../api'
...
export const useGeneratorStore = defineStore('generator', {
  ...
  actions: {
    ...
    updateProgress(index: number, status: 'generating' | 'done' | 'error', url?: string, error?: string) {
      ...
      if (image) {
        image.status = status
        if (url) image.url = withAuthUrl(url)
    ...
    updateImage(index: number, newUrl: string) {
      ...
      if (image) {
        ...
        image.url = withAuthUrl(`${newUrl}${separator}t=${timestamp}`)
        image.status = 'done'
        delete image.error
      }
    },
