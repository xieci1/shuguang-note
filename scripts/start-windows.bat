...
:: 启动后端 (新窗口，蓝色背景)
if %USE_UV% equ 1 (
    start "薯光笔记-后端-12398" cmd /k "color 1F && title 薯光笔记 后端服务 [12398] && uv run python backend/app.py"
) else (
    start "薯光笔记-后端-12398" cmd /k "color 1F && title 薯光笔记 后端服务 [12398] && python backend/app.py"
)
...
:: 启动前端 (新窗口，绿色背景)
cd frontend
start "薯光笔记-前端-5173" cmd /k "color 2F && title 薯光笔记 前端服务 [5173] && %PKG_MANAGER% run dev"
cd ..
