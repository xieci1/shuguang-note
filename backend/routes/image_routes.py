...
def create_image_blueprint():
    ...
    @require_user()
    def generate_images(current_user):
        """
        批量生成图片（SSE 流式返回）
        ...
        - quality_mode: fast 或 fine，fast 会跳过封面参考图以提升速度
        ...
        try:
            ...
            if task_id and not get_history_service().can_access_task(task_id, current_user):
            ...
            try:
                get_user_service().consume_quota(current_user["id"], len(pages))
            except ValueError as quota_error:
                return jsonify({"success": False, "error": str(quota_error)}), 403
    ...
    def get_image(task_id, filename):
        ...
        try:
            ...
            if not get_history_service().can_access_task(task_id, current_user):
                return jsonify({"success": False, "error": "无权访问该图片"}), 403

            # 检查是否请求缩略图
            thumbnail = request.args.get('thumbnail', 'true').lower() == 'true'

            history_root = get_history_service().history_dir
    ...
    @require_user()
    def retry_single_image(current_user):
        """
        重试生成单张失败的图片
        ...
        try:
            ...
            if not get_history_service().can_access_task(task_id, current_user):
                return jsonify({"success": False, "error": "无权访问该任务"}), 403
            try:
                get_user_service().consume_quota(current_user["id"], 1)
            except ValueError as quota_error:
                return jsonify({"success": False, "error": str(quota_error)}), 403
    ...
    @require_user()
    def retry_failed_images(current_user):
        """
        批量重试失败的图片（SSE 流式返回）
    ...
    @require_user()
    def regenerate_image(current_user):
        """
        重新生成图片（即使成功的也可以重新生成）
    ...
    @require_user()
    def get_task_state(current_user, task_id):
        """
        获取任务状态
        ...
        try:
            ...
            if state is None:
                record_id = get_history_service().find_record_id_by_task(task_id)
                if record_id:
                    record = get_history_service().get_record(record_id)
                    generated = {}
                    failed = {}
                    for index, filename in enumerate(record.get("images", {}).get("generated", [])):
                        if filename:
                            generated[index] = filename
                    state = {"generated": generated, "failed": failed, "cover_image": None}
