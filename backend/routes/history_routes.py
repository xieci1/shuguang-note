...
def create_history_blueprint():
    ...
    def create_history(current_user):
        ...
        try:
            ...
            record_id = history_service.create_record(topic, outline, task_id, user_id=current_user["id"])
    ...
    def list_history(current_user):
        ...
        try:
            ...
            history_service = get_history_service()
            result = history_service.list_records(page, page_size, status, current_user=current_user, user_id=user_id)
    ...
    def get_history(current_user, record_id):
        ...
        try:
            history_service = get_history_service()
            record = history_service.get_record(record_id, current_user=current_user)
    ...
    @require_user()
    def check_history_exists(current_user, record_id):
        """
        检查历史记录是否存在
        ...
        try:
            history_service = get_history_service()
            exists = history_service.record_exists(record_id, current_user=current_user)
    ...
    @require_user()
    def update_history(current_user, record_id):
        """
        更新历史记录
    ...
    def delete_history(current_user, record_id):
        ...
        try:
            history_service = get_history_service()
            success = history_service.delete_record(record_id, current_user=current_user)
    ...
    def search_history(current_user):
        ...
        try:
            ...
            history_service = get_history_service()
            results = history_service.search_records(keyword, current_user=current_user)
    ...
    def get_history_stats(current_user):
        ...
        try:
            history_service = get_history_service()
            stats = history_service.get_statistics(current_user=current_user)
    ...
    @history_bp.route('/tasks', methods=['GET'])
    @require_user()
    def list_tasks(current_user):
        ...
        try:
            ...
            return jsonify({
                "success": True,
                "tasks": history_service.list_tasks(limit=limit, current_user=current_user)
            }), 200
    ...
    @require_user()
    def scan_task(current_user, task_id):
        """
        扫描单个任务并同步图片列表
        ...
        try:
            history_service = get_history_service()
            if current_user.get("role") != "admin" and not history_service.can_access_task(task_id, current_user):
    ...
    @require_user()
    def scan_all_tasks(current_user):
        """
        扫描所有任务并同步图片列表
    ...
    @history_bp.route('/history/repair', methods=['POST'])
    @require_user()
    def repair_history(current_user):
        """
        诊断并修复历史记录数据不一致。

        请求体：
        - dry_run: true 只预览修复动作；false 执行修复
        """
        try:
            ...
            if current_user.get("role") != "admin":
                return jsonify({"success": False, "error": "需要管理员权限"}), 403
            history_service = get_history_service()
            result = history_service.repair_records(dry_run=bool(dry_run))
            return jsonify(result), 200
    ...
    @require_user()
    def download_history_zip(current_user, record_id):
        """
        下载历史记录的所有图片为 ZIP 文件
