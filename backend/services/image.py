...
class ImageService:
    ...
    def generate_images(
        ...
        Args:
            ...
            quality_mode: fast 跳过封面参考图并直接并发，fine 保持封面参考图流程
        ...
        use_fast_mode = quality_mode != "fine"
        logger.info(f"开始图片生成任务: task_id={task_id}, pages={len(pages)}, quality_mode={quality_mode}")
        history_service = get_history_service()
        history_service.mark_task_started(task_id, pages)
        ...
        if use_fast_mode:
            yield {
                ...
                "data": {
                    "status": "batch_start",
                    "message": f"快速模式：并发生成 {len(pages)} 页图片...",
                    "current": 0,
                    "total": total,
                    "phase": "content"
                }
                ...
                for future in as_completed(future_to_page):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            history_service.mark_image_result(task_id, index, filename)

                            if index == 0:
                                cover_path = os.path.join(self.current_task_dir, filename)
                                if os.path.exists(cover_path):
                                    with open(cover_path, "rb") as f:
                                        self._task_states[task_id]["cover_image"] = compress_image(f.read(), max_size_kb=200)
                            ...
                            history_service.mark_image_result(task_id, index, None, error)
                        ...
                        history_service.mark_image_result(task_id, page["index"], None, error_msg)
    ...
    def retry_single_image(
        ...
        if success:
            get_history_service().mark_image_result(task_id, index, filename)
            if task_id in self._task_states:
                self._task_states[task_id]["generated"][index] = filename
            ...
            get_history_service().mark_image_result(task_id, index, None, error)
            return {
                "success": False,
                "index": index,
                "error": error,
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_CONCURRENT) as executor:
            ...
            for future in as_completed(future_to_page):
                ...
                try:
                    ...
                    failed_count += 1
                    get_history_service().mark_image_result(task_id, page["index"], None, str(e))
                    yield {
                        "event": "error",
                        "data": {
                            "index": page["index"],
