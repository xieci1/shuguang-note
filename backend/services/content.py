...
class ContentService:
    ...
    def generate_tags(self, title: str, body: str, count: int = 8) -> Dict[str, Any]:
        """根据发布标题和正文生成小红书标签。"""
        try:
            prompt = (
                "你是小红书运营助手。请根据下面的标题和正文，生成适合小红书发布的中文标签。\n"
                f"要求：返回 JSON，格式为 {{\"tags\": [\"标签1\", \"标签2\"]}}；数量 {count} 个以内；"
                "标签不要包含 #，不要有空格，不要解释。\n\n"
                f"标题：{title}\n\n正文：{body[:3000]}"
            )
            active_provider = self.text_config.get('active_provider', 'google_gemini')
            ...
            response_text = self.client.generate_text(
                ...
                max_output_tokens=min(provider_config.get('max_output_tokens', 1200), 1200),
            )
            ...
            cleaned = []
            for tag in tags:
                value = str(tag).strip().lstrip("#").replace(" ", "")
                if value and value not in cleaned:
                    cleaned.append(value)
            return {"success": True, "tags": cleaned[:count]}
        except Exception as e:
